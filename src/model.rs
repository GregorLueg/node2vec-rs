...
impl<B: Backend> SkipGramModel<B> {
    ...
    pub fn forward(
        ...
        let targets_2d: Tensor<B, 2, Int> = targets.unsqueeze_dim(1);
        let contexts_2d: Tensor<B, 2, Int> = contexts.unsqueeze_dim(1);

        let target_embed: Tensor<B, 2> = self.target_embd.forward(targets_2d).squeeze::<2>();
        let context_embed: Tensor<B, 2> = self.context_embd.forward(contexts_2d).squeeze::<2>();

        // pos score: [batch_size, embedding_dim] -> [batch_size, 1] -> [batch_size]
        let pos_dot: Tensor<B, 2> = (target_embed.clone() * context_embed).sum_dim(1);
        let pos_score: Tensor<B, 1> = activation::sigmoid(pos_dot.squeeze::<1>());

        let neg_embed: Tensor<B, 3> = self.context_embd.forward(negatives);
        let target_expanded: Tensor<B, 3> = target_embed.unsqueeze_dim(1);

        // neg scores: [batch_size, num_neg, embedding_dim] -> [batch_size, num_neg]
        let neg_dot: Tensor<B, 2> = (target_expanded * neg_embed).sum_dim(2).squeeze::<2>();
        let neg_scores: Tensor<B, 2> = activation::sigmoid(neg_dot);

        // loss
        let pos_loss: Tensor<B, 1> = pos_score.log().neg();
        let neg_loss: Tensor<B, 1> = (neg_scores.neg() + 1.0)
            .log()
            .sum_dim(1)
            .squeeze::<1>()
            .neg();
